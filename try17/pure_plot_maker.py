...
def plot_x_values_over_time(points3D, fps):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('./output/Change in X Value Over Time')
...
def plot_y_values_over_time(points3D, fps):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('./output/Change in Y Value Over Time')
...
def plot_z_values_over_time(points3D, fps):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('./output/Change in Z Value Over Time')
...
def plot_distances_over_time(distances, fps, start_frame):
    ...
    for marker_idx, marker_distances in enumerate(distances):
        ...
        #smoothed_distances = gaussian_filter1d(interpolated_distances, sigma=2)  # 데이터 평활화
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig('./output/Displacement from Origin')
