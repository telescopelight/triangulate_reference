...
# 재투영 오차 계산 함수
def reprojection_error_3d(params, image_points1, image_points2, camera_matrix1, camera_matrix2, rvec1, tvec1, rvec2, tvec2):
    points_3d = params.reshape(-1, 3)  # N x 3
    
    # 첫 번째 카메라로부터의 재투영 좌표 계산
    proj_points1, _ = cv2.projectPoints(points_3d, rvec1, tvec1, camera_matrix1, distCoeffs=None)
    proj_points1 = proj_points1.squeeze()
    
    # 두 번째 카메라로부터의 재투영 좌표 계산
    proj_points2, _ = cv2.projectPoints(points_3d, rvec2, tvec2, camera_matrix2, distCoeffs=None)
    proj_points2 = proj_points2.squeeze()
    
    # 재투영 오차 계산
    error1 = image_points1 - proj_points1
    error2 = image_points2 - proj_points2
    
    # 두 오차를 합쳐서 반환 (벡터 형태로 반환해야 함)
    return np.hstack((error1.ravel(), error2.ravel()))

#각각의 점들을 삼각측량 기법을 이용해 3차원 점 좌표로 변환(Z좌표 추정)
def triangulate_3d_points(left_centroids, right_centroids, calib_data_left, calib_data_right, R, T):
    ...
    for i, (left_frame_centroids, right_frame_centroids) in enumerate(zip(left_centroids, right_centroids)):
        if len(left_frame_centroids) == len(right_frame_centroids):
            ...
            # 비선형 최적화 적용
            initial_params = np.array(frame_points_3D).flatten()
            optimized_result = least_squares(
                reprojection_error_3d,
                initial_params,
                args=(left_frame_centroids, right_frame_centroids,
                      calib_data_left['camera_matrix'], calib_data_right['camera_matrix'],
                      np.zeros(3), np.zeros(3), R, T)
            )
            optimized_3d_points = optimized_result.x.reshape(-1, 3)

            points_3D_all_frames.append(optimized_3d_points)
        else:
...
def run():
    ...
    if left_centroids and right_centroids:
        points_3D_all_frames = triangulate_3d_points(left_centroids, right_centroids, left_calib, right_calib, R, T)
