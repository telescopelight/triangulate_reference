...
def interpolate_nan_values(values):
    ...
    if not np.any(~nans):
        return values  # 모든 값이 NaN일 경우 보간 불가
    indices = np.arange(len(values))
    return np.interp(indices, indices[~nans], values[~nans])

def plot_x_values_over_time(points3D, fps):
...
def plot_y_values_over_time(points3D, fps):
    num_markers = max(len(frame) for frame in points3D)
    time_values = np.arange(len(points3D)) / fps
...
def plot_z_values_over_time(points3D, fps):
    num_markers = max(len(frame) for frame in points3D)
    time_values = np.arange(len(points3D)) / fps
    ...
    for marker_idx in range(num_markers):
        ...
        for peak in peaks:
            plt.text(time_values[peak], interpolated_z_values[peak], f'{interpolated_z_values[peak]:.2f}', color='green', fontsize=3, ha='right')
        for trough in troughs:
            plt.text(time_values[trough], interpolated_z_values[trough], f'{interpolated_z_values[trough]:.2f}', color='red', fontsize=3, ha='right')
...
def plot_distances_over_time(distances, fps, start_frame):
    ...
    for marker_idx, marker_distances in enumerate(distances):
        ...
        for peak in peaks:
            plt.text(time_values[peak], interpolated_distances[peak], f'{interpolated_distances[peak]:.2f}', color='green', fontsize=3, ha='right')
        for trough in troughs:
            plt.text(time_values[trough], interpolated_distances[trough], f'{interpolated_distances[trough]:.2f}', color='red', fontsize=3, ha='right')

        plt.xlabel('Time (s)')
        plt.ylabel('Displacement (mm)')
        plt.ylim(-max_distance, max_distance)
        plt.title(f'Displacement from Origin Over Time for Marker {marker_idx + 1}')
        plt.grid(True)
        plt.legend()
